...
def record_result(guidance_name, step, score, std, pipeline_name, env_name,diffusion_ckpt, critic_ckpt):
    ...
    log_csv = f"log_results_{pipeline_name}_{env_name}_{timestamp}_D_GFDT.csv"
...
def save_results_to_csv(results_dict, pipeline_name, env_name,seed):
    ...
    # timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    output_csv = f"guidance_scores_{pipeline_name}_{env_name}_{timestamp}_seed{seed}_D_GFDT.csv"
...
def pipeline(args):
    ...
    fallback_dir = f'seed123_dql/{args.pipeline_name}/{args.task.env_name}/pretrained'
