...
def pipeline(args):
    ...
    experiment_tag = f"baseline_{timestamp}"
    save_path = f'results/{args.pipeline_name}_{args.seed}/{args.task.env_name}/{experiment_tag}/'
    ...
    print(">>> after creation, affine_in weight device =", next(nn_diffusion.affine_in.parameters()).device)
    ...
    if args.mode == "train":
        ...
        for batch in loop_dataloader(dataloader):
            ...
            # ----------- Saving ------------
            if ((n_gradient_step + 1) % args.save_interval == 0) or ((n_gradient_step + 1) % 400 == 0 and n_gradient_step<100000):
