...
def get_ckpts_with_fallback(guidance_dir1, guidance_dir2):
    diffusion_ckpts = {
        extract_step(f1): os.path.join(guidance_dir1, f1)
        for f1 in os.listdir(guidance_dir1)

        if f1.startswith("diffusion_ckpt_") and f1.endswith(".pt") and extract_step(f1) is not None
    }

    critic_ckpts = {        
        extract_step(f1): os.path.join(guidance_dir2, f1)
        for f1 in os.listdir(guidance_dir2)
...
def record_result(guidance_name, step, score, std, pipeline_name, env_name,diffusion_ckpt, critic_ckpt):
    ...
    log_csv = f"log_results_{pipeline_name}_{env_name}_{timestamp}_D_baseline.csv"
...
def pipeline(args):
    ...
    save_path2 = f'seed123_dql/{args.pipeline_name}/{args.task.env_name}/'
    ...
    # guidance_dirs = [sorted(get_guidance_dirs(save_path1))[0],sorted(get_guidance_dirs(save_path2))[0]]
    ...
    step_ckpt_pairs = get_ckpts_with_fallback(g_dir1,g_dir2)
