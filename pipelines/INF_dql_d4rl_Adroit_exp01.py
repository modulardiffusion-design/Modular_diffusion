...
def record_result(guidance_name, step, score, std, pipeline_name, env_name,diffusion_ckpt, critic_ckpt):
    ...
    log_csv = f"log_results_{pipeline_name}_{env_name}_{timestamp}_baseline_exp01.csv"
...
def save_results_to_csv(results_dict, pipeline_name, env_name):
    ...
    # timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    output_csv = f"guidance_scores_{pipeline_name}_{env_name}_{timestamp}_baseline_exp01.csv"
...
def get_guidance_dirs(base_path):
    return [os.path.join(base_path, d) for d in os.listdir(base_path)
            if os.path.isdir(os.path.join(base_path, d)) and "pretrained" not in d]
...
@hydra.main(config_path="../configs/dql/adroit_exp01", config_name="adroit", version_base=None)
def pipeline(args):
    set_seed(args.seed)
